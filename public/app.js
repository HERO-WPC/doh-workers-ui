...
(() => {
  ...
  async function refreshDashboard() {
    ...
    // 各面板独立容错(含运行统计):单个面板异常只影响自己,并在面板内提示原因:单个面板的数据/渲染异常不再连累其它面板
    const safe = (id, fn) => {
      try { fn(); } catch (e) {
        console.error(`[面板 ${id}] 渲染失败`, e);
        const node = el(id);
        if (node) node.innerHTML = `<span class="error">渲染失败: ${escapeHtml(e && e.message ? e.message : String(e))}</span>`;
      }
    };

    safe("stats-cards", () => {
      if (!stats.isolate) throw new Error("/stats 未返回 isolate 字段");
    ...
    safe("upstream-health", () => {
      if (!stats.upstreams) throw new Error("/stats 未返回 upstreams 字段");
