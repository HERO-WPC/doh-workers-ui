...
本 Worker 跑在 Cloudflare 边缘,单次查询的耗时大致是:

```
客户端 → 优选/反代 IP(SNI 转发)→ CF 边缘 → Worker(缓存或上游)
```

其中 Worker 自身开销只有几毫秒,**大头是"客户端到 CF 边缘"这一段——完全由你选的优选 IP / 反代质量决定**。同一份代码,优选 IP 选得好就是十几毫秒,选得差就是几百毫秒甚至超时。务必用 `tools/` 里的测活脚本对候选 IP 做真实 DoH 探测后再上线,并定期复测更换(参考下文「优选 IP」)。

以下是优选 IP 质量较好时的全国实测参考(itdog.cn,296 个监测点,对 DoH 域名发起真实查询):

![itdog 全国 DoH 查询测速](docs/itdog-speedtest.png)

- 全部节点平均 **0.669s**(含建立连接的首查),最快的节点(中国香港)**0.034s**;
- 绝大多数省份 ≤0.5s(绿色),缓存命中时日常体感为几十毫秒级;
- 最慢的个别地区(如浙江电信 3.5s)通常是跨网 + 冷缓存的首查,不代表日常水平。
